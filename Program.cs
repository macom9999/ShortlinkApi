...
// ---- Konfiguration (Env-Variablen bevorzugt) ----
var connString = builder.Configuration["PG_CONN"]
                 ?? Environment.GetEnvironmentVariable("PG_CONN")
                 ?? throw new Exception("DB conn string missing (PG_CONN).");
...
// CORS: für die Extension großzügig (MVP). Später einschränken.
...
// ---- Auto-Migration beim Start (wichtig für Render/Neon) ----
...
// Zusätzlicher DB-Healthcheck
app.MapGet("/__db", async (AppDb db) =>
    await db.Database.CanConnectAsync()
        ? Results.Ok(new { db = "ok" })
        : Results.Problem("db-fail"));

// Helper: Kurzcode erzeugen (Base62, ~8 Zeichen)
string MakeCode()
{
...
app.MapPost("/links", async (AppDb db, HttpRequest req) =>
{
    ...
    if (string.IsNullOrWhiteSpace(longUrl))
        return Results.BadRequest(new { error = "long_url required" });
...
// ---- GET /r/{code}  -> 302 Redirect + Klick-Log ----
...
// ---- GET /stats/{code} -> { code, long_url, total_clicks } ----
